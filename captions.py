...
FPS = 30
MAX_OUTPUT_DIM = 1920  # cap the longer edge — no need to process at full 12MP+ photo res
...
def render_clip(input_image: str, out_path: str, preset_name: str, work_dir: Path):
    ...
    # Cap the working resolution — phone photos can be 12MP+, but we're
    # outputting a short social clip, not a full-res video. Processing at
    # full photo resolution was the main reason this was slow.
    scale = min(1.0, MAX_OUTPUT_DIM / max(orig_w, orig_h))
    img_w = int(orig_w * scale) // 2 * 2  # keep even for libx264
    img_h = int(orig_h * scale) // 2 * 2
